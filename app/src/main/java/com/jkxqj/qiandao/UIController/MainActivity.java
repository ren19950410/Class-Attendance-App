...
import com.jkxqj.qiandao.model.YanZheng;
...
//获取MAC
    ...
    public void Dao(View view) {
        final String Yanzheng=newpassword_et.getText().toString();
        ...
        {
            BmobQuery<YanZheng> query=new BmobQuery<YanZheng>();
            query.addWhereEqualTo("Yanzheng",Yanzheng);
            query.findObjects(MainActivity.this, new FindListener<YanZheng>()
            {
                @Override
                public void onSuccess(List<YanZheng> Yanzhengs)
                {
                    if(Yanzhengs.size()==0)
                    {
                        Toast.makeText(MainActivity.this,"验证码不存在！",Toast.LENGTH_LONG).show();
                        return;
                    }
                    for(YanZheng a:Yanzhengs)
                    {
                        if(!a.getYanzheng().equals(Yanzheng))
                        {
